...
const TestSections = {
  ...
  renderChecklist(job) {
    ...
    const specificLabel = job.reportType === 'generator' ? 'Generator-Specific Sections' : 'Synchronous Motor-Specific Sections';
    const groups = [
    ...
    return groups.map((g) => `
      ...
      ${g.sections.map((s) => this.renderSectionToggle(job, s)).join('')}
    `).join('');
  },

  renderSectionToggle(job, section) {
    ...
    return `
      <div class="test-section${checked ? ' expanded' : ''}" data-test-section="${section.key}">
        ...
        ${checked ? `<div class="test-section-body" data-test-body="${section.key}">${this.renderSectionBody(section, state.values || {})}</div>` : ''}
      </div>
    `;
  },
